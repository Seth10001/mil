...
parser = argparse.ArgumentParser(description='Generates the points down the centerline of a reace track given specified by a yaml file')
# get input yaml specifying racetrack
parser.add_argument('input_file', help='name of the input yaml file specifying the racetrack')
# get output file name
parser.add_argument('output_file', help='location of output xacro file')
# get debug plot or not
parser.add_argument('--debug', action='store_true', help='if present, will plot the points of the track')
...
def points_as_complex(pose, radius, center_dist, resolution):
  ...
  orientation = complex(np.cos((90 - azimuth)*np.pi/180), np.sin((90 - azimuth)*np.pi/180)) * 1j
  center_of_rotation = complex(pose['orientation']['center_of_rotation']['x'],
                               pose['orientation']['center_of_rotation']['y'])
  translation = complex(pose['translation']['x'], pose['translation']['y'])

  points = np.zeros((int(2 * np.pi * radius * resolution),), dtype=np.complex)
...
def points_as_str(*args):
  ...
  for i in range(points.shape[0]):
    ret += '    <point> ' + str(points[i,0]) + ' ' + str(points[i,1]) + ' ' + str(points[i,2]) + '</point>\n'
  return ret
...
if __name__ == '__main__':
  ...
  out += points_as_str(track['pose'], track['radius'], track['center_dist'], track['resolution'])
  out += '    </road>\n'
  out += '  </xacro:macro>'
  out += '</world>\n'
